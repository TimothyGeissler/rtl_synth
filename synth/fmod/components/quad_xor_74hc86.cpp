 ...
 * @brief Functional simulation of 74HC86 Quad XOR Logic Gate IC
 * 
 * The 74HC86 is a quad 2-input XOR gate IC in a 14-pin DIP package.
 * This class provides a functional simulation of the device including:
 * - Four independent XOR gates
...
private:
    ...
    // Propagation delay in nanoseconds (typical for 74HC86)
    ...
    // Array of the four XOR gates
    std::vector<Gate> gates;

public:
    /**
     * @brief Constructor - Initialize the 74HC86 IC
     */
    QuadXOR_74HC86() : powerOn(false) {
        // Initialize all pins to floating state
        for (int pin = 1; pin <= 14; pin++) {
            pinStates[static_cast<Pin>(pin)] = FLOATING;
        }
        
        // Set up the four XOR gates according to pinout
        gates = {
    ...
    void printGateStates() const {
        ...
        for (size_t i = 0; i < gates.size(); i++) {
            ...
            std::cout << gate.name << ": ";
            std::cout << logicLevelToString(inputA) << " XOR " << logicLevelToString(inputB);
        ...
        for (int gateNum = 1; gateNum <= 4; gateNum++) {
            ...
            for (LogicLevel a : testInputs) {
                for (LogicLevel b : testInputs) {
                    ...
                    LogicLevel expected = xorLogic(a, b);
...
private:
    ...
    void updateOutputs() {
        ...
        for (const auto& gate : gates) {
            ...
            // XOR logic: output is HIGH when inputs are different
            LogicLevel output = xorLogic(inputA, inputB);
            pinStates[gate.output] = output;
        }
    }
    
    /**
     * @brief XOR logic function
     * @param a First input
    ...
    LogicLevel xorLogic(LogicLevel a, LogicLevel b) const {
        ...
        // XOR: HIGH when inputs are different, LOW when inputs are same
        return (a != b) ? HIGH : LOW;
    }
...
int main() {
    std::cout << "74HC86 Quad XOR Gate Functional Simulation\n";
    std::cout << "==========================================\n\n";
    
    // Create an instance of the 74HC86
    QuadXOR_74HC86 ic;
    ...
    // Test Gate 1: 0 XOR 0 = 0
    ic.setGateInputs(1, QuadXOR_74HC86::LOW, QuadXOR_74HC86::LOW);
    std::cout << "Gate 1: 0 XOR 0 = " << ic.getGateOutput(1) << "\n";
    
    // Test Gate 1: 0 XOR 1 = 1
    ic.setGateInputs(1, QuadXOR_74HC86::LOW, QuadXOR_74HC86::HIGH);
    std::cout << "Gate 1: 0 XOR 1 = " << ic.getGateOutput(1) << "\n";
    
    // Test Gate 1: 1 XOR 0 = 1
    ic.setGateInputs(1, QuadXOR_74HC86::HIGH, QuadXOR_74HC86::LOW);
    std::cout << "Gate 1: 1 XOR 0 = " << ic.getGateOutput(1) << "\n";
    
    // Test Gate 1: 1 XOR 1 = 0
    ic.setGateInputs(1, QuadXOR_74HC86::HIGH, QuadXOR_74HC86::HIGH);
    std::cout << "Gate 1: 1 XOR 1 = " << ic.getGateOutput(1) << "\n\n";
    
    // Test all gates simultaneously
    std::cout << "Testing all gates simultaneously:\n";
    std::cout << "=================================\n";
    
    ic.setGateInputs(1, QuadXOR_74HC86::LOW, QuadXOR_74HC86::HIGH);   // Gate 1: 0 XOR 1 = 1
    ic.setGateInputs(2, QuadXOR_74HC86::HIGH, QuadXOR_74HC86::LOW);   // Gate 2: 1 XOR 0 = 1
    ic.setGateInputs(3, QuadXOR_74HC86::HIGH, QuadXOR_74HC86::HIGH);  // Gate 3: 1 XOR 1 = 0
    ic.setGateInputs(4, QuadXOR_74HC86::LOW, QuadXOR_74HC86::LOW);    // Gate 4: 0 XOR 0 = 0

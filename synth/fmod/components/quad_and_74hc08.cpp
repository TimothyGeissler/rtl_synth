 ...
 * @brief Functional simulation of 74HC08 Quad AND Logic Gate IC
 * 
 * The 74HC08 is a quad 2-input AND gate IC in a 14-pin DIP package.
...
private:
    ...
    // Propagation delay in nanoseconds (typical for 74HC08)
    ...
    // Array of the four AND gates
    std::vector<Gate> gates;

public:
    /**
     * @brief Constructor - Initialize the 74HC08 IC
     */
    QuadAND_74HC08() : powerOn(false) {
        // Initialize all pins to floating state
        for (int pin = 1; pin <= 14; pin++) {
            pinStates[static_cast<Pin>(pin)] = FLOATING;
        }
        
        // Set up the four AND gates according to pinout
        gates = {
    ...
    void printGateStates() const {
        ...
        for (size_t i = 0; i < gates.size(); i++) {
            ...
            std::cout << gate.name << ": ";
            std::cout << logicLevelToString(inputA) << " AND " << logicLevelToString(inputB);
        ...
        for (int gateNum = 1; gateNum <= 4; gateNum++) {
            ...
            for (LogicLevel a : testInputs) {
                for (LogicLevel b : testInputs) {
                    ...
                    LogicLevel expected = andLogic(a, b);
...
private:
    ...
    void updateOutputs() {
        ...
        for (const auto& gate : gates) {
            ...
            // AND logic: output is HIGH only when both inputs are HIGH
            LogicLevel output = andLogic(inputA, inputB);
            pinStates[gate.output] = output;
        }
    }
    
    /**
     * @brief AND logic function
     * @param a First input
...
int main() {
    std::cout << "74HC08 Quad AND Gate Functional Simulation\n";
    std::cout << "==========================================\n\n";
    
    // Create an instance of the 74HC08
    QuadAND_74HC08 ic;
    ...
    // Test Gate 1: 0 AND 0 = 0
    ic.setGateInputs(1, QuadAND_74HC08::LOW, QuadAND_74HC08::LOW);
    std::cout << "Gate 1: 0 AND 0 = " << ic.getGateOutput(1) << "\n";
    
    // Test Gate 1: 0 AND 1 = 0
    ic.setGateInputs(1, QuadAND_74HC08::LOW, QuadAND_74HC08::HIGH);
    std::cout << "Gate 1: 0 AND 1 = " << ic.getGateOutput(1) << "\n";
    
    // Test Gate 1: 1 AND 0 = 0
    ic.setGateInputs(1, QuadAND_74HC08::HIGH, QuadAND_74HC08::LOW);
    std::cout << "Gate 1: 1 AND 0 = " << ic.getGateOutput(1) << "\n";
    
    // Test Gate 1: 1 AND 1 = 1
    ic.setGateInputs(1, QuadAND_74HC08::HIGH, QuadAND_74HC08::HIGH);
    std::cout << "Gate 1: 1 AND 1 = " << ic.getGateOutput(1) << "\n\n";
    
    // Test all gates simultaneously
    std::cout << "Testing all gates simultaneously:\n";
    std::cout << "=================================\n";
    
    ic.setGateInputs(1, QuadAND_74HC08::LOW, QuadAND_74HC08::HIGH);   // Gate 1: 0 AND 1 = 0
    ic.setGateInputs(2, QuadAND_74HC08::HIGH, QuadAND_74HC08::LOW);   // Gate 2: 1 AND 0 = 0
    ic.setGateInputs(3, QuadAND_74HC08::HIGH, QuadAND_74HC08::HIGH);  // Gate 3: 1 AND 1 = 1
    ic.setGateInputs(4, QuadAND_74HC08::LOW, QuadAND_74HC08::LOW);    // Gate 4: 0 AND 0 = 0

 ...
 * @brief Functional simulation of 74HC00 Quad NAND Logic Gate IC
 * 
 * The 74HC00 is a quad 2-input NAND gate IC in a 14-pin DIP package.
 * This class provides a functional simulation of the device including:
 * - Four independent NAND gates
...
private:
    ...
    // Propagation delay in nanoseconds (typical for 74HC00)
    ...
    // Array of the four NAND gates
    std::vector<Gate> gates;

public:
    /**
     * @brief Constructor - Initialize the 74HC00 IC
     */
    QuadNAND_74HC00() : powerOn(false) {
        // Initialize all pins to floating state
        for (int pin = 1; pin <= 14; pin++) {
            pinStates[static_cast<Pin>(pin)] = FLOATING;
        }
        
        // Set up the four NAND gates according to pinout
        gates = {
    ...
    void printGateStates() const {
        ...
        for (size_t i = 0; i < gates.size(); i++) {
            ...
            std::cout << gate.name << ": ";
            std::cout << logicLevelToString(inputA) << " NAND " << logicLevelToString(inputB);
        ...
        for (int gateNum = 1; gateNum <= 4; gateNum++) {
            ...
            for (LogicLevel a : testInputs) {
                for (LogicLevel b : testInputs) {
                    ...
                    LogicLevel expected = nandLogic(a, b);
...
private:
    ...
    void updateOutputs() {
        ...
        for (const auto& gate : gates) {
            ...
            LogicLevel output = nandLogic(inputA, inputB);
            pinStates[gate.output] = output;
        }
    }
    
    /**
     * @brief NAND logic function
     * @param a First input
    ...
    LogicLevel nandLogic(LogicLevel a, LogicLevel b) const {
        ...
        // NAND: LOW only when both inputs are HIGH, otherwise HIGH
...
int main() {
    std::cout << "74HC00 Quad NAND Gate Functional Simulation\n";
    std::cout << "===========================================\n\n";
    
    // Create an instance of the 74HC00
    QuadNAND_74HC00 ic;
    ...
    // Test Gate 1: 0 NAND 0 = 1
    ic.setGateInputs(1, QuadNAND_74HC00::LOW, QuadNAND_74HC00::LOW);
    std::cout << "Gate 1: 0 NAND 0 = " << ic.getGateOutput(1) << "\n";
    
    // Test Gate 1: 0 NAND 1 = 1
    ic.setGateInputs(1, QuadNAND_74HC00::LOW, QuadNAND_74HC00::HIGH);
    std::cout << "Gate 1: 0 NAND 1 = " << ic.getGateOutput(1) << "\n";
    
    // Test Gate 1: 1 NAND 0 = 1
    ic.setGateInputs(1, QuadNAND_74HC00::HIGH, QuadNAND_74HC00::LOW);
    std::cout << "Gate 1: 1 NAND 0 = " << ic.getGateOutput(1) << "\n";
    
    // Test Gate 1: 1 NAND 1 = 0
    ic.setGateInputs(1, QuadNAND_74HC00::HIGH, QuadNAND_74HC00::HIGH);
    std::cout << "Gate 1: 1 NAND 1 = " << ic.getGateOutput(1) << "\n\n";
    
    // Test all gates simultaneously
    std::cout << "Testing all gates simultaneously:\n";
    std::cout << "=================================\n";
    
    ic.setGateInputs(1, QuadNAND_74HC00::LOW, QuadNAND_74HC00::HIGH);   // Gate 1: 0 NAND 1 = 1
    ic.setGateInputs(2, QuadNAND_74HC00::HIGH, QuadNAND_74HC00::LOW);   // Gate 2: 1 NAND 0 = 1
    ic.setGateInputs(3, QuadNAND_74HC00::HIGH, QuadNAND_74HC00::HIGH);  // Gate 3: 1 NAND 1 = 0
    ic.setGateInputs(4, QuadNAND_74HC00::LOW, QuadNAND_74HC00::LOW);    // Gate 4: 0 NAND 0 = 1
